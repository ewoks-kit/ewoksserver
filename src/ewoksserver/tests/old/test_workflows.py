def test_single_workflow(rest_client_old):
    ...
    response = rest_client_old.post("/workflows", json=workflow1a)
    ...
    response = rest_client_old.put(f"/workflow/{identifier}", json=workflow1b)
    data = response.get_json()
    assert response.status_code == 200, data
    ...
    response = rest_client_old.delete(f"/workflow/{identifier}")
    data = response.get_json()
...
def test_multiple_workflows(rest_client_old, default_workflow_identifiers):
    response = rest_client_old.get("/workflows")
    data = response.get_json()
    assert response.status_code == 200
    ...
    response = rest_client_old.post("/workflows", json=workflow1b)
    data = response.get_json()
    assert response.status_code == 409, data
    assert data["message"] == "Workflow 'myworkflow1' already exists."
    response = rest_client_old.post("/workflows", json=workflow2)
    data = response.get_json()
    assert response.status_code == 200, data
...
def test_workflow_descriptions(rest_client_old, default_workflow_identifiers):
    response = rest_client_old.get("/workflows/descriptions")
    data = response.get_json()
    assert response.status_code == 200
    ...
    workflow2 = {"graph": {"id": "myworkflow2"}, "nodes": [{"id": "task1"}]}
    response = rest_client_old.post("/workflows", json=workflow1)
    data = response.get_json()
    assert response.status_code == 200, data
    response = rest_client_old.post("/workflows", json=workflow2)
    data = response.get_json()
...
def test_workflow_description_keys(rest_client_old, default_workflow_identifiers):
    desc = {
        "id": "myworkflow1",
        "label": "label1",
        "category": "cat1",
...
def test_workflow_keywords(rest_client_old, default_workflow_identifiers):
