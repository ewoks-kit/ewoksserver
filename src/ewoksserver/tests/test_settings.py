...
def test_ewoks_execution_no_discovery(tmp_path):
    filename = tmp_path / "config.py"
    write_config(
        filename,
...
def test_deprecated_ewoks_field(tmp_path):
    filename = tmp_path / "config.py"
    write_config(
        filename,
        RESOURCE_DIRECTORY=str(tmp_path),
...
def test_ignore_deprecated_ewoks_field(tmp_path):
    filename = tmp_path / "config.py"
    write_config(
        filename,
        RESOURCE_DIRECTORY=str(tmp_path),
...
def test_deprecated_timeout_field(tmp_path):
    filename = tmp_path / "config.py"
    write_config(filename, RESOURCE_DIRECTORY=str(tmp_path), DISCOVER_TIMEOUT=100)
...
def test_ignore_deprecated_timeout_field(tmp_path):
    filename = tmp_path / "config.py"
    write_config(
        filename,
        RESOURCE_DIRECTORY=str(tmp_path),
...
def test_ewoks_local_scheduling(tmp_path):
    filename = tmp_path / "config.py"
...
def test_ewoks_celery_scheduling(tmp_path):
    filename = tmp_path / "config.py"
    celery_config = {
